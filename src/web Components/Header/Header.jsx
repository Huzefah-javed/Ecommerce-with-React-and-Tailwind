   ...
   const handleExperienceMenuActive =()=> {
    ...
    <header className="w-full flex justify-between items-center absolute z-30 top-0 mt-10">
                    <div className={`${location.pathname === "/"? " ml-8": "h-10"} relative flex items-center justify-start gap-0 -skew-x-[30deg]`}>
                        ...
                        <div className={`${location.pathname === "/"? "skew-x-[30deg]": ` ${location.pathname.includes("/products/")? "bg-white":"bg-black"} h-full flex items-center justify-start pl-12 p-3>`}`}>
                        <img className={`${location.pathname === "/"? "w-72": "w-8 skew-x-30"}`} src={`${location.key === "default"? "main-logo.png": location.pathname.includes("/products/")? "/Logo.png":"logo-white.png"}`} alt="main logo" />
                    </div>
                        {location.key === "default"?"":<><div onClick={()=>navigator(-1)} className={` flex justify-center items-center ${location.pathname.includes("/products/")? "bg-white text-black":"bg-black text-white"} h-full px-4 text-2xl cursor-pointer`}><IoIosArrowBack className="skew-x-30"/></div>
